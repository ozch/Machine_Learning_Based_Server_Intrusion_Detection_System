...
class DataProcess:
    ...
    def prepareInstance(self,instance):
        ...
        if(sr[1].startswith("icmp")):
            ...
            df = ['duration', 'src_bytes', 'dst_bytes', 'count', 'srv_count','class']
            ...
            df_temp = df
            df_temp.update({'class':0})
            writer.writerow(df_temp)
            protocol = 1
        else:
            if(sr[2].startswith("http")):
                csv_file = open('data/HTTP.csv', 'a+')
                df = ['duration','flag','src_bytes', 'dst_bytes', 'urgent', 'wrong_fragment', 'class']
                writer = csv.DictWriter(csv_file, fieldnames=df)
                # HTTP FEED : duration,flag,src_bytes,dst_bytes,urgent,wrong_fragment
                ...
                df = ['duration','src_bytes', 'dst_bytes', 'count', 'srv_count','diff_srv_rate' ,'dst_host_count','dst_host_same_src_port_rate','class']
                writer = csv.DictWriter(csv_file, fieldnames=df)
                ...
                df = ['duration', 'flag', 'src_bytes', 'dst_bytes', 'count', 'srv_count', 'class']
                writer = csv.DictWriter(csv_file, fieldnames=df)
